...
import { Button, EmptyState, Notice, PageHeader, PageShell, Section, SegmentedControl, StatCard } from './ui';
...
export default function PerformanceSummary({ compact = false, className = '' }) {
  const { user } = useAuth();
  ...
  const stats = (
    <section className="ui-stat-grid performance-summary-stats">
  ...
  const chart = loading ? (
    ...
    <div className="performance-bar-chart" style={{ '--bar-count': chartRows.length }}>
      {chartRows.map((row) => {
        const rowActions = metricValue(row, 'totalActions') || metricValue(row, 'keystrokeCount') + metricValue(row, 'mouseClickCount');
        const height = Math.max(8, Math.round((rowActions / maxActions) * 150));
        return (
          <div key={row.period} className="performance-bar-item">
            <div className="performance-bar-value">{formatNum(rowActions)}</div>
  ...
  if (compact) {
    return (
      <Section
        className={cx('performance-summary performance-summary--compact', className)}
        title="Hiệu suất gần đây"
        description="Tổng hợp tuần/tháng từ dữ liệu đã gom, đặt chung với Theo dõi để tránh phải mở thêm trang."
        actions={actions}
      >
        {error && (
        ...
        {stats}
        <div className="performance-summary-chart">
          {chart}
        </div>
      </Section>
    );
  ...
  return (
    <PageShell className={cx('performance-page', className)} narrow mono>
      <PageHeader
        icon={BarChart3}
        eyebrow="Performance"
